...
def options(opt):
    ...
    opt.add_option('--freedesktop', action='store_true', default=False, dest='freedesktop',
                    help='Ignored by the headless Zephyr core build')
...
def build(bld):
    ...
    bld.path.find_dir ('libs/ctrl-interface/control_protocol/control_protocol')
...
def test(bld):
    subprocess.call("headless/hardev")

def help2man(bld):
    set_version ()
    cmd = "help2man -s 1 -N -o ardour.1 -n Ardour --version-string='Ardour %s' headless/hardev" % PROGRAM_VERSION
